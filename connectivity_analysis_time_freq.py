            ...
            for condition_type in condition:
                ...
                for hemi in hemisphere:
                    ...
                    con = spectral_connectivity_epochs(
                        ...
                        method= con_method,
                        mode = "cwt_morlet",
                        cwt_freqs = np.arange(freq_min,freq_max,1),
                        cwt_n_cycles = con_n_cycles,
                        tmin = tmin_con,
                        tmax = tmax_con,
                        faverage = False, 
                        sfreq=sfreq,
                    ...
                    np.save(os.path.join(connectivity_output_dir,savefile),con.get_data())

                    #plot time-frequency plot
                    sub_fig,sub_ax1 = plt.subplots(figsize=(6.4,4.8), layout='constrained')
                    pc = sub_ax1.pcolormesh(epochs.times,con.freqs,np.squeeze(con.get_data()),vmin = vmin, vmax = vmax)
                ...
                report.add_figure(fig=fig1, title=title, section=participant, tags=[diagnosis,condition_type])
            ...
            for sub_id in participants_list:
                if sub_id in participants_to_study:
                    if sub_id not in exclude_participants:
                        ...
                        savefile = '_'.join([participant,diagnosis,condition_type,hemi +'.npy'])
                        try:
                            participant_data = np.load(os.path.join(savedir,savefile))
                            plotdata.append(np.squeeze(participant_data))
                        except:
                            print('participant is not in this diagnosis group')
            plotdata = np.array(plotdata)
            plotdata_ave = np.average(plotdata,axis=0).shape
            #plot time-frequency plot
            sub_fig,sub_ax1 = plt.subplots(figsize=(6.4,4.8), layout='constrained')
            pc = sub_ax1.pcolormesh(epochs.times,con.freqs,np.squeeze(con.get_data()),vmin = vmin, vmax = vmax)
            #save temporary plot for report
