...
for participant in participants:
    ...
    stc_miso = mne.read_source_estimate(load_fname,subject=subjID_date).savgol_filter(30)
    
    condition = 'Novels'
    condition_name = condition + '-lh.stc'
    load_fname = find_files(condition_name,data_dir)[0]
    stc_novel = mne.read_source_estimate(load_fname,subject=subjID_date).savgol_filter(30)
    ...
    for hemi in hemispheres:
        ...
        for label in labels:
            ROI = label[0]
            stc_miso_label  = stc_miso.in_label(label[1])
            stc_novel_label  = stc_novel.in_label(label[1])
            for time_window in time_windows:
                peak_vertex,peak_time = stc_miso_label.get_peak(hemi = hemi, tmin = time_window[1],tmax = time_window[2],vert_as_index = True, time_as_index = True)
                if hemi == 'lh':
                    peak_value_tw_miso = stc_miso_label.lh_data[peak_vertex,:]
                else:
                    peak_value_tw_miso = stc_miso_label.rh_data[peak_vertex,:]
                
                peak_value_abs_miso = np.abs(peak_value_tw_miso)
                data = [participant[0],participant[1],'Misophone',hemi,ROI,time_window[0],peak_value_abs_miso]
                participants_data.append(data)
                
                if hemi == 'lh':
                    peak_value_tw_novel = stc_novel_label.lh_data[peak_vertex,:]
                else:
                    peak_value_tw_novel = stc_novel_label.rh_data[peak_vertex,:]
                
                peak_value_abs_novel = np.abs(peak_value_tw_novel)
                data = [participant[0],participant[1],'Novels',hemi,ROI,time_window[0],peak_value_abs_novel]
                participants_data.append(data)
...
df.to_csv('ROI_time_series_abs.csv',index=False)
